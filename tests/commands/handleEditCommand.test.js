...
describe('handleEditCommand', () => {
  ...
  describe('successful updates with new structure', () => {
    ...
    it('should update welcomeMessage in new structure', async () => {
      ...
      expect(mockServices.dataService.loadData).toHaveBeenCalledTimes(2); // Initial load + reload
    });
  ...
  describe('backward compatibility', () => {
    ...
    it('should check new structure first, then fall back to old structure for verification', async () => {
      // Mock getValue to return undefined for new structure but found in old
      mockServices.dataService.getValue.mockImplementation((key) => {
        if (key === 'editableMessages.welcomeMessage') return undefined;
        if (key === 'welcomeMessage') return 'Updated welcome message';
        return undefined;
      });
